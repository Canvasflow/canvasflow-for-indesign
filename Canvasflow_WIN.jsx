...
var commandFilePath = "~/canvasflow_runner.bat";
...
var CanvasflowBuild = function(settingsPath, commandFilePath) {
    ...
    $.resizeImages = function(imageFiles) {
        ...
        dataFile.writeln('@echo off');
        dataFile.writeln('setlocal enabledelayedexpansion');

        for(var i = 0; i < imageFiles.length; i++) {
            dataFile.writeln('set Files['+i+']="'+imageFiles[i]+'"');
        }
        
        dataFile.writeln('for /l %%n in (0,1,' + (imageFiles.length - 1) + ') do (');
        dataFile.writeln('\tset original_image=!Files[%%n]!');
        dataFile.writeln('\tset ext=""');
        dataFile.writeln('\tset parent_dir=""');
        dataFile.writeln('\tset filename=""');

        dataFile.writeln('\tfor %%i in (!original_image!) do set ext=%%~xi');
        dataFile.writeln('\tfor %%a in (!original_image!) do set parent_dir=%%~dpa');
        dataFile.writeln('\tfor %%f in (!original_image!) do set filename=%%~nf');

        dataFile.writeln('\tset image_width_command="magick identify -ping -format \'%%w\' !original_image!"');

        dataFile.writeln('\tset image_width=""');
        dataFile.writeln('\tfor /f "delims=" %%a in (\'!image_width_command!\') do set image_width=%%a');

        dataFile.writeln('\tset target_filename="!parent_dir!!filename!.jpg"');
        dataFile.writeln('\tif !image_width! gtr 2048 (');
        dataFile.writeln('\t\tmagick convert -colorspace sRGB -geometry 2048x !original_image! -quality 50 !target_filename!');
        dataFile.writeln('\t) else (');
        dataFile.writeln('\t\tmagick convert -colorspace sRGB !original_image! -quality 50 !target_filename!');
        dataFile.writeln('\t)');
        ...
        dataFile.writeln('del %userprofile%\\canvasflow_resizing.lock');
    ...
    $.convertImages = function(imageFiles) {
        ...
        dataFile.writeln('del %userprofile%\\canvasflow_convert.lock');
